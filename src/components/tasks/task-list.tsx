...
import { Calendar, ChevronUp, ChevronDown, ChevronsUpDown, AlertCircle } from 'lucide-react'
          ...
          <tr className="border-b border-gray-100 bg-gray-50">
            {cols.map(c => (
              <th key={c.key} className={c.cls}>
                ...
                >
                  {c.label}
                  <SortIcon field={c.key} active={sortKey} dir={sortDir} />
                </button>
              </th>
            ))}
          ...
          {sorted.map(task => {
            ...
            return (
              <tr key={task.id} className="hover:bg-gray-50 transition-colors group">
                <td className="px-4 py-3">
                  <Link href={`/tasks/${task.id}`} className="flex items-start gap-2 min-w-0">
                    ...
                    <div className="min-w-0">
                      <p className="font-medium text-gray-900 group-hover:text-indigo-600 transition-colors truncate max-w-xs">
                        {task.title}
                      </p>
                      {task.tags?.length > 0 && (
                ...
                <td className="px-4 py-3">
                  {task.due_date ? (
                    <span className={cn('flex items-center gap-1 text-xs', overdue && 'text-red-600 font-medium', dueSoon && 'text-amber-600 font-medium', !overdue && !dueSoon && 'text-gray-500')}>
